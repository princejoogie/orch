...
Use opencode SDK endpoints when they exist. For example, project tabs come from `client.project.list()`, project worktree options come from `client.project.directories()` after a bounded best-effort `client.v2.projectCopy.refresh()` for projects with sessions in the two-day recent session window, selected-project sessions come from `client.session.list()`, and pending permission requests come from `client.permission.list()`. Project snapshots are filtered to projects with sessions whose `projectID` matches the project row and sorted by latest session activity before reaching UI code. Project session rows are cached per project, then the dashboard controller applies the active worktree filter before rendering the session list. Session row latest-message previews use the v2 session messages endpoint with descending order so the table previews the newest visible user or assistant message; the prompt dialog separately loads full history for its scrollable message list. Dashboard data refresh is primarily event-driven through OpenCode's global event stream (`client.global.event()`), with slower query polling left as a fallback for missed events or older servers.
