...
- The new-session dialog owns focus state between its textarea and worktree selector. `tab` changes dialog focus; selector navigation keys are scoped out while the textarea is focused, `enter` commits the focused selector back to the textarea, and `dd` opens removal confirmation for a focused non-primary worktree.
