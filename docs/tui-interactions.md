...
- Dialog hover should not commit side-effectful choices. For example, hovering `+ New worktree` in the new-session dialog must not change the pending worktree or model/provider state; click or keyboard navigation commits that selection.
